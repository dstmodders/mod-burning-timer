...
_G.mod_burningTimer.campfireReveal = GetModConfigData("showCampfireTimer") == "hidden" and 0.0
    or nil
...
local function fetchBurntime(prefab)
    ...
    math.random = function(a, b)
        return a and b and a or a and 1 or 0.0
    end -- Set random to lowest value possible
    ...
    if burntimeList[prefab] then -- boats do technically burn forever
        math.random = function(a, b)
            return a and b and b or a and a or 1.0
        end -- Set random to highest value possible
        inst = _G.SpawnPrefab(prefab)
        if inst and inst.components.burnable and inst.components.burnable.burntime then
...
local function fetchCampfireStats(prefab)
    ...
    if inst and inst.components.fueled and inst.components.fueled.rate then
        ...
        campfireFireLevels[prefab] = inst.components.burnable
    ...
    if _G.mod_burningTimer.debug then
        print(
            "Fetched Campfire Stats from",
            prefab,
            "- Max Fuel:",
            campfireMaxFuel[prefab],
            "Fuel Rate:",
            campfireFuelRate[prefab],
            "Rain Rate:",
            campfireRainRate[prefab],
            "Fire Levels:",
...
local function fetchLanternStats(prefab)
    if lanternLightTime[prefab] then
        return false
    end
    ...
    if
        ...
        and (inst.components.fueled.updatefn or inst.components.fueled.ontakefuelfn)
    then
    ...
    if _G.mod_burningTimer.debug then
        print(
            "Fetched Lantern Stats from",
            prefab,
            "- Light Time:",
            lanternLightTime[prefab],
            "Intensity Min:",
            lanternIntensityMin[prefab],
            "Intensity Diff:",
            lanternIntensityDiff[prefab]
        )
    end
    return true
end
...
local function getBurntime(prefab)
    ...
    if burntimeList[prefab] == nil then
        fetchBurntime(prefab)
    end
    if not burntimeList[prefab] then
        return 0.0, nil
    end
...
local function getCampfireStats(prefab)
    ...
    if campfireMaxFuel[prefab] == nil then
        fetchCampfireStats(prefab)
    end
    if not campfireMaxFuel[prefab] then
        return false, 0.0, 0.0, {}
    end
    return campfireMaxFuel[prefab],
        campfireFuelRate[prefab],
        campfireRainRate[prefab],
        campfireFireLevels[prefab]
end
...
local function getLanternStats(prefab)
    ...
    if lanternLightTime[prefab] == nil then
        fetchLanternStats(prefab)
    end
    if not lanternLightTime[prefab] then
        return false, 0.0, 0.0
    end
        ...
        if InGame() and _G.TheWorld and _G.mod_burningTimer then
            ...
            if _G.ThePlayer and _G.ThePlayer.components and _G.ThePlayer.components.talker then
                _G.ThePlayer.components.talker:Say(
                    "Burning Timer: " .. (_G.mod_burningTimer.enabled and "Enabled" or "Disabled")
                )
            end
        end
    end)
...
if GetModConfigData("showCampfireTimer") then
    AddPrefabPostInit("campfirefire", function(inst)
        return campfireTimer(inst)
    end)
    AddPrefabPostInit("coldfirefire", function(inst)
        return campfireTimer(inst)
    end)
    ...
    AddPrefabPostInit("obsidianfirefire", function(inst)
        return campfireTimer(inst)
    end) -- From Tropical Experience mod
end
...
if GetModConfigData("showLanternTimer") then
    AddPrefabPostInit("lanternlight", function(inst)
        return lanternTimer(inst)
    end)
end
...
if
    GetModConfigData("showCampfireTimer") == "hidden"
    or GetModConfigData("showLanternTimer") == "hidden"
    or GetModConfigData("showStarTimer") == "hidden"
then
