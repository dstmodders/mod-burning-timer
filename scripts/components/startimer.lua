...
local StarTimer = Class(function(self, inst)
    ...
    self.correction = 0.0
    self.duration = inst.prefab == "stafflight" and TUNING.YELLOWSTAFF_STAR_DURATION
        or TUNING.OPALSTAFF_STAR_DURATION
...
function StarTimer:MakeTimer()
    ...
    self.widget = ThePlayer.HUD.overlayroot:AddChild(
        FollowText(BODYTEXTFONT, mod_burningTimer.starTextSize)
    )
