...
local BurningTimer = Class(function(self, inst)
    ...
    if not inst.parent then -- That would be too easy
        local entities = TheSim:FindEntities(
            ...
            { "fire" },
            { "INLIMBO", "bt_invalid" }
        ) -- Note: pos.y needs to be included since Red Hounds do not always spawn their fires at y = 0
...
function BurningTimer:TimeLeft()
    return self.duration + self.spawntime - GetTime()
end
...
function BurningTimer:PassTimer()
    ...
    for k, v in pairs(entities) do
        ...
        if num >= 10 then
            break
        end -- Stop looking for others after taking 10 requests to prevent lag from a stack of burning items
...
function BurningTimer:MakeTimer()
    ...
    self.widget = ThePlayer.HUD.overlayroot:AddChild(
        FollowText(BODYTEXTFONT, mod_burningTimer.burntimeTextSize)
    )
...
function BurningTimer:AdjustColour()
    ...
    elseif remaining > 0.0 then
        ...
        self.widget.text:SetColour(5 / 3 - num, 1.5 - num * 2, 1.0 - num * 3 / 2, 1)
    elseif self.rng then
        local num = -remaining / self.rng
        self.widget.text:SetColour(2 / 3 + num * 1 / 3, num * 2 / 3, num * 2 / 3, 1)
    else
        self.widget.text:SetColour(2 / 3, 0, 0, 1)
    end
end
...
function BurningTimer:UpdateTimer(dt)
    ...
    if self.rng then
        text = string.format(
            "%s\n(%0.2fs)",
            text,
            math.max(self.rng - math.max(time_passed - self.duration, 0.0), 0)
        )
    end
